...
def fft_walsh_hadamard_from_definition(x, duration, example_signal=False):
    """
    Perform Walsh-Hadamard Transform (WHT) using matrix definition.

    Args:
    - x (list or np.array): Input signal in the time domain.
    - duration (float): Duration of the signal (not used in WHT).
    - example_signal (bool): If true, use internal test signal.
    ...
    def hadamard_matrix(m):
        if m == 0:
            return np.array([[1]])
        Hm_1 = hadamard_matrix(m - 1)
        top = np.hstack((Hm_1, Hm_1))
        bottom = np.hstack((Hm_1, -Hm_1))
        return (1 / np.sqrt(2)) * np.vstack((top, bottom))
...
def fft_walsh_hadamard(x, duration, example_signal=False):
    """
